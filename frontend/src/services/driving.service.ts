...
export async function resolveDestinationWithGoogleMaps(
  ...
  try {
    ...
    try {
      ...
      const predictions = await autocompleteRequest(autocompleteService, {
        input: query,
        componentRestrictions: { country: "br" },
        bounds: buildBounds(
          googleMaps,
          deviceLocation,
          isPreciseAddress ? 0.28 : 0.18
        ),
        language: "pt-BR",
        region: "br",
      });
    ...
    try {
      const results = await geocodeRequest(geocoder, {
        address: query,
        region: "BR",
        componentRestrictions: { country: "BR" },
        bounds: isPreciseAddress
          ? undefined
          : buildBounds(googleMaps, deviceLocation, 0.28),
      });

      const resolved = mapGeocoderResult(results[0], "geocode");
      if (resolved) return resolved;
    } catch {
      // continua fluxo
    }
    ...
    console.log("Erro resolver destino cliente:", error);
    return null;
  }
}